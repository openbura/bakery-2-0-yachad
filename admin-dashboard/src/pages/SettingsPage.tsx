...
type SettingsPageProps = {
  ...
  onSave: (settings: StoreSettings) => Promise<StoreSettings>;
};

const noticeOptions: Array<{ value: NoticeType; label: string; description: string }> = [
  { value: 'info', label: 'מידע', description: 'עדכון כללי ללקוחות' },
...
export function SettingsPage({ settings, onSave }: SettingsPageProps) {
  ...
  const save = async () => {
    ...
    try {
      ...
      setSaveError(ownerMessage(error, 'השינויים לא נשמרו. בדקו את החיבור ונסו שוב.'));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="page-stack settings-page">
        ...
        <div className="settings-main">
          <section className="settings-section">
            ...
            <div className="settings-controls">
              <Toggle checked={draft.orderingEnabled} onChange={(value) => setDraft((current) => ({ ...current, orderingEnabled: value }))} label="הזמנות פתוחות" description="כאשר סגור, הקטלוג נשאר גלוי אך אי אפשר להזמין" disabled={saving} />
              <Toggle checked={draft.deliveryEnabled} onChange={(value) => setDraft((current) => ({ ...current, deliveryEnabled: value }))} label="משלוחים פעילים" description="ניתן להשהות משלוחים ולהשאיר איסוף עצמי פעיל" disabled={saving} />
              <Toggle checked={draft.pickupEnabled} onChange={(value) => setDraft((current) => ({ ...current, pickupEnabled: value }))} label="איסוף עצמי פעיל" description="שליטה נפרדת מאפשרות המשלוח" disabled={saving} />
            </div>
          ...
          <section className="settings-section">
            ...
            <label className="textarea-field" htmlFor="customer-notice"><span>תוכן ההודעה</span><textarea id="customer-notice" value={draft.noticeText} onChange={(event) => setDraft((current) => ({ ...current, noticeText: event.target.value }))} disabled={!draft.noticeActive || saving} maxLength={160} rows={4} /><small><bdi dir="ltr">{draft.noticeText.length}/160</bdi></small></label>
      ...
      <div className="sticky-save-bar" aria-busy={saving}><div><strong>{saveError || (dirty ? 'יש שינויים שממתינים לשמירה' : 'ההגדרות מעודכנות')}</strong><small>{saveError ? 'השינויים נשארו בטופס ולא סומנו כשמורים.' : dirty ? 'בדקו את התצוגה המקדימה לפני השמירה' : `עודכן ${settings.updatedAt}`}</small></div><button className="button button--primary" type="button" disabled={!dirty || invalidAvailability || invalidNotice || saving} onClick={() => void save()}>{saving ? <><span className="spinner" /> שומר…</> : <><FloppyDisk size={19} /> שמירת שינויים</>}</button>{saveError && <span className="sr-only" role="alert">{saveError}</span>}</div>
    </div>
  );
}
